...
sub eachFile{
  ...
  if (-e $filename) { 
    if($fullpath =~ m/\d+X\/merSize\d+\/polishing\d+\/data\/consensus\.fasta$/){
      if($fullpath =~ m/(\d+X).*(merSize\d+).*(polishing\d+)/){
...
my @string;
$string[0] = "#Unpolished_assembly_name";
...
@string = 1;
$string[0] = "#Polished_assembly_name";
# Then loop through result hash and print results in a table
$i = 1;
