...
Gbam <- list(readGAlignments(paste(folderExtract,sample,".scOEAMAP.bam",sep=""),param=paramFirst,use.names=TRUE), readGAlignments(paste(folderExtract,sample,".scOEAMAP.bam",sep=""),param=paramSecond,use.names=TRUE))
...
selectScOEAinsert <- function(file, param, idSca, type, first) {

  scOEASca <- readGAlignments(file, param=param,use.names=TRUE)
  seqlevels(scOEASca) <- as.character(idSca)

  idSC <- 1
  if(first) { idSC <- 2 }
  bamRef <- Gbam[[idSC]][which(names(Gbam[[idSC]]) %in% names(scOEASca))]

  bamRef <- bamRef[order(names(bamRef))]
  scOEASca <- scOEASca[order(names(scOEASca))]

# ##########################################################
# Récupère la partie du OEA qui est sur la reference
# ##########################################################

  if (length(scOEASca) > length(bamRef)) {
      read <- as.data.frame(table(names(scOEASca)), stringsAsFactors=FALSE)
      read2 <- read[which(read$Freq>1),]
      for (i in 1:length(read2[,1])){
	if (read2$Freq[i]==2) {
	  iRead <- grep(read2$Var1[i], names(scOEASca), fixed=TRUE)
	  badOne <- iRead[which(qwidth(scOEASca[iRead])==min(qwidth(scOEASca[iRead])))]
	  scOEASca <- scOEASca[-badOne]
	}else{
	  scOEASca <- scOEASca[-which(names(scOEASca)==read2$Var1[i])]
  ...
  if (length(scOEASca) != length(bamRef)) {# Shouldn't append but ...
    stop(paste("error 2 : number of scOEASca is different. Need to improve the R script. idSca :",idSca , sep=""))
  }

  if(length(scOEASca)==0){return(NULL)}
...
# #####################################
  ...
  tabInsert <- data.frame(NAME=names(bamRef), CHR=as.vector(seqnames(bamRef)), REF_START=start(bamRef), REF_END=end(bamRef), REF_POS=rep(NA,length(end(bamRef))), REF_STRAND=as.vector(strand(bamRef)), SCA_ID=rep(idSca,length(end(scOEASca))), SCA_START=start(scOEASca), SCA_END=end(scOEASca), SCA_POS=rep(NA,length(end(scOEASca))), SCA_STRAND=as.vector(strand(scOEASca)), SCA_READ_FIRST=rep(first,length(end(scOEASca))), stringsAsFactors=FALSE) 
...
for (i in 1:length(tabSca$Scaffold)) {
  ...
  C_ScOEA1 <- selectScOEAinsert(paste(folderScaffold,"cov/scOEAUNMAP.1.bam",sep=""), param, idSca, "scOEA", TRUE)
  C_ScOEA2 <- selectScOEAinsert(paste(folderScaffold,"cov/scOEAUNMAP.2.bam",sep=""), param, idSca, "scOEA", FALSE)
  cluster <- rbind(C_ScOEA1, C_ScOEA2)

# no efficient
#   cmd=expression(selectScOEAinsert(paste(folderScaffold,"cov/scOEAUNMAP.1.bam",sep=""), param, idSca, "scOEA", TRUE))
#   mcparallel(cmd)
#   cmd=expression(selectScOEAinsert(paste(folderScaffold,"cov/scOEAUNMAP.2.bam",sep=""), param, idSca, "scOEA", FALSE))
#   mcparallel(cmd)
#   tmp <- mccollect()
...
if(!is.null(allCluster)){
  write.table(allCluster, file=paste(folderOut,"cluster.scOEA.tab",sep=""), sep="\t", dec = ".", row.names = FALSE, col.names= TRUE, quote=FALSE)
  ...
  write.table(allCluster2, file=paste(folderOut,"cluster.scOEA.fusion.tab",sep=""), sep="\t", dec = ".", row.names = FALSE, col.names= TRUE, quote=FALSE)

  #o <- which(abs(allCluster$REF_END - allCluster$REF_START) < meanReadLen)
  #if (length(o)!=0){
  #  allCluster <- allCluster[-o,]
  #}

  #write.table(allCluster, file=paste(folderOut,"cluster.scOEA.filter.tab",sep=""), sep="\t", dec = ".", row.names = FALSE, col.names= TRUE, quote=FALSE)
}
