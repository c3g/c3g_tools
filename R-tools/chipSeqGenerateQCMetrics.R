...
sample_name=readset$Sample[i]
mark_name=readset$MarkName[i]
...
barplot(d2[2:10,2], names.arg=d2[2:10,1],ylim=c(0,1),xlab="Number of Tags per Genomic Position", ylab="Fraction of Genomic Positions", main=paste(sample_name, "\nTag count: ", round(mean,3), " avg tag per position", sep=" "), col="blue")
...
plot(d1[,1], d1[,2], xlab="Distance from reference tag (bp)", ylab="Number of observations", xlim=c(-300,400), ylim=c(0,maxH+500), main=paste(sample_name, " \nTag autocorrelation", sep=" "), col="blue", type='l')
...
d1<-read.table(countDist, header=T, sep="\t")
plot(d1[,1], d1[,2], xlab="Distance from 5' end of tags", ylab="Nucleotide Frequency", col="blue", xlim=c(-50,100), ylim=c(0,0.45), type='l', main=paste(sample_name, "\nSequence bias", sep=" "))
lines(d1[,1],d1[,3], col="red")
...
plot(d1[,1], d1[,3], col="black", type='l', xlab="GC content of ChIP fragments", ylim=c(0,maxH),ylab="Normalized Fraction of total fragments", main=paste(sample_name, "\n GC bias", sep=" "))
