...
for(i in 1:nrow(readsets)) {
    ...
    countDist<-paste(output_dir, "/tags", sample_name[i], mark_name[i], "tagCountDistribution.txt", sep="/")
    ...
    barplot(d2[2:10,2], names.arg=d2[2:10,1],ylim=c(0,1),xlab="Number of Tags per Genomic Position", ylab="Fraction of Genomic Positions", main=paste(sample_name[i], mark_name[i], "\nTag count: ", round(mean,3), " avg tag per position", sep=" "), col="blue")

    # Tag auto correlation

    countDist<-paste(output_dir, "/tags", sample_name[i], mark_name[i], "tagAutocorrelation.txt", sep="/")
    d1<-read.table(countDist, header=T, sep="\t")
    maxH<-max(max(d1[,2]),max(d1[,3]))

    plot(d1[,1], d1[,2], xlab="Distance from reference tag (bp)", ylab="Number of observations", xlim=c(-300,400), ylim=c(0,maxH+500), main=paste(sample_name[i], mark_name[i], " \nTag autocorrelation", sep=" "), col="blue", type='l')
    lines(d1[,1], d1[,3], col="red")
    legend("topleft", c("same strand","opposite strand"), col=c("blue", "red"), pch='-', pt.cex=3,bty='n') 

    # Sequence bias

    countDist<-paste(output_dir, "/tags", sample_name[i], mark_name[i], "tagFreq.txt", sep="/")
    d1<-read.table(countDist, header=T, sep="\t")
    plot(d1[,1], d1[,2], xlab="Distance from 5' end of tags", ylab="Nucleotide Frequency", col="blue", xlim=c(-50,100), ylim=c(0,0.45), type='l', main=paste(sample_name[i], mark_name[i], "\nSequence bias", sep=" "))
    lines(d1[,1],d1[,3], col="red")
    ...
    countGenome<-paste(output_dir, "/tags", sample_name[i], mark_name[i], "tagGCcontent.txt", sep="/")
    countIP<-paste(output_dir, "/tags", sample_name[i], mark_name[i], "genomeGCcontent.txt", sep="/")
    d1<-read.table(countGenome, header=T, sep="\t")
    d2<-read.table(countIP, header=T, sep="\t")

    maxH<-max(max(d1[,3]),max(d2[,3]))

    plot(d1[,1], d1[,3], col="black", type='l', xlab="GC content of ChIP fragments", ylim=c(0,maxH),ylab="Normalized Fraction of total fragments", main=paste(sample_name, mark_name[i], "\n GC bias", sep=" "))
