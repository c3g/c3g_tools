...
MAPPED_READS=$(samtools view -c -F 2308 $SAMPLE_PATH) 

N_NO_MULTIMAP=$(samtools view -b -c -F 2304 $SAMPLE_PATH)
...
samtools view -c -F 2304 $SAMPLE_PATH -L $INTERGENIC_BED | awk -v MAPPED_READS=$MAPPED_READS '{print "FRACTION_INTERGENIC\t"$1/MAPPED_READS}' >> ${OUTPUT_DIR}/${SAMPLE_NAME}.read_stats.txt

#get the number of reads from rRNA with samtools
samtools view -c -F 2304 $SAMPLE_PATH -L $RRNA_BED | awk -v MAPPED_READS=$MAPPED_READS '{print "FRACTION_RRNA\t"$1/MAPPED_READS}' >> ${OUTPUT_DIR}/${SAMPLE_NAME}.read_stats.txt
