...
if [ $TARGET_FLAG == 1 ]; then
  echo -e "sample\traw_reads\ttrimmed_reads\t%_survivalrate\taln_reads\tMappingEfficiency\tDuplicatedReads\t%aligned_duplicate\tDeduplicatedAlignRreads\t%_UsefulAlignRate\t%Proportion_Unique_filteredReads\tOntargetReads\t%Ontarget\t%onTargetvsRawRead\tpUC19_meth\tlambdaConversion\thumanConversion\tmean_genomecoverage\tmedianCpGcoverage\t#_CG_1X\t#_CG_10X\t#_CG_30X" > $TABLE_OUTFILE
else
  echo -e "sample\traw_reads\ttrimmed_reads\t%_survivalrate\taln_reads\tMappingEfficiency\tDuplicatedReads\t%aligned_duplicate\tDeduplicatedAlignRreads\t%_UsefulAlignRate\t%Proportion_Unique_filteredReads\tpUC19_meth\tlambdaConversion\thumanConversion\tmean_genomecoverage\tmedianCpGcoverage\t#_CG_1X\t#_CG_10X\t#_CG_30X" > $TABLE_OUTFILE
fi
...
do
  ...
  genomecoverage=`sed 1d alignment/$sample/$sample.sorted.dedup.all.coverage.sample_summary | awk '{print $3}' | head -1`
  ...
  if [ $TARGET_FLAG == 1 ]; then
    OntargetReads=`cat alignment/$sample/$sample.sorted.dedup.ontarget.bam.flagstat|grep mapped|head -1|awk '{printf "%d",$1/2}'`
  ...
  DedupMQ10filteredreads=`cat alignment/$sample/$sample.sorted.dedup.filtered_reads.counts.txt`
  ...
  MedianCpGcov=`cat methylation_call/$sample/$sample.readset_sorted.dedup.median_CpG_coverage.txt|head -1 |awk '{print $NF}'`

  # GC bias, the absolute value of the correlation score should not exceed 0.4. IHEC required.
  GCbias=`cat alignment/$sample/$sample.sorted.dedup.GCBias_all.txt|tail -1`
  ...
  puc19Meth=`cat methylation_call/$sample/$sample.*pUC19*.txt`
  ...
  TotalmCHG=`cat alignment/$sample/$sample*/$sample.*sorted_noRG_bismark_bt2_PE_report.txt|grep "Total methylated C's in CHG context:"|awk '{print $NF}'|awk '{sum+=$1;} END {printf "%d\n", sum}'`
  TotalmCHH=`cat alignment/$sample/$sample*/$sample.*sorted_noRG_bismark_bt2_PE_report.txt|grep "Total methylated C's in CHH context:"|awk '{print $NF}'|awk '{sum+=$1;} END {printf "%d\n", sum}'`
  TotalumCHG=`cat alignment/$sample/$sample*/$sample.*sorted_noRG_bismark_bt2_PE_report.txt|grep "Total unmethylated C's in CHG context:"|awk '{print $NF}'|awk '{sum+=$1;} END {printf "%d\n", sum}'`
  TotalumCHH=`cat alignment/$sample/$sample*/$sample.*sorted_noRG_bismark_bt2_PE_report.txt|grep "Total unmethylated C's in CHH context:"|awk '{print $NF}'|awk '{sum+=$1;} END {printf "%d\n", sum}'`
  ...
  if [ $TARGET_FLAG == 1 ]; then
    echo -e "$sample\t$rawReads\t$trimmedReads\t$SurvivalRate\t$AlignedReads\t$MappingEfficiency\t$DuplicateReads\t$DuplicationRate\t$DeduplicatedAlignRreads\t$UsefulAlignRate\t$DedupMQ10Proportion\t$OntargetReads\t$OntargetRate\t$onTargetvsRawRead\t$puc19Meth\t$lambdaConversion\t$humanConversion\t$genomecoverage\t$MedianCpGcov\t$cg1x\t$cg10x\t$cg30x"
  else
    echo -e "$sample\t$rawReads\t$trimmedReads\t$SurvivalRate\t$AlignedReads\t$MappingEfficiency\t$DuplicateReads\t$DuplicationRate\t$DeduplicatedAlignRreads\t$UsefulAlignRate\t$DedupMQ10Proportion\t$puc19Meth\t$lambdaConversion\t$humanConversion\t$genomecoverage\t$MedianCpGcov\t$cg1x\t$cg10x\t$cg30x"
  fi
