...
echo -e "genome_assembly\tChIP_type\tmark_name\tinput_name\tmark_raw_reads\tmark_trimmed_reads\tmark_trimmed_frac\tmark_mapped_reads\tmark_mapped_frac\tmark_dup_reads\tmark_dup_frac\tmark_filtered_reads\tmark_filtered_frac\tmark_MT_reads\tmark_MT_frac\tinput_raw_reads\tinput_trimmed_reads\tinput_trimmed_frac\tclt_mapped_reads\tinput_mapped_frac\tinput_dup_reads\tinput_dup_frac\tinput_filtered_reads\tinput_filtered_frac\tinput_MT_reads\tinput_Mt_frac\tnmb_peaks\treads_in_peaks\tfrip\tmark_nsc\tinput_nsc\tmark_rsc\tinput_rsc\tmark_Quality\tinput_Quality\tsingletons\tjs_dist\tchance_div\n" > ${OUTPUT_DIR}/IHEC_metrics_chipseq_${SAMPLE_NAME}.${CHIP_NAME}.txt
